...
type Update struct {
	...
	cachedInfo string `json:"-"`
}

func (u *Update) Info() string {
	if u.cachedInfo != "" {
		return u.cachedInfo
	}
	u.cachedInfo = fmt.Sprintf("id [%d]" , u.UpdateID,)
	if from := u.FromChat(); from != nil {
		u.cachedInfo = fmt.Sprintf(u.cachedInfo + " from user [%d] Name [%s %s] UserName [%s]",  from.ID, from.FirstName, from.LastName, from.UserName  )
		if u.Message != nil {
	...
	u.cachedInfo = fmt.Sprintf(u.cachedInfo + " updateType [%s]", mgtype)
	return u.cachedInfo
}
...
type Message struct {
	...
	MessageOrigin *MessageOrigin `json:"forward_origin,omitempty"`
...
func (m *Message) IsForwaded() bool {
	return m.ForwardFrom != nil || m.ForwardFromChat != nil
}
...
type WebhookInfo struct {
	...
	Secret_token string `json:"secret_token,omitempty"`

	content     []byte `json:"-"`
	firstcalled bool   `json:"-"`
}

func (info *WebhookInfo) Close() error {
	return nil
}
func (info *WebhookInfo) Read(p []byte) (int, error) {
	if !info.firstcalled {
		var err error
		info.content, err = json.Marshal(info)
		if err != nil {
			return 0, err
		}
	...
	n := copy(p, info.content)

	info.content = info.content[n:]
...
type InlineKeyboardButtonv2 struct {
	Text         string `json:"text,omitempty"`
...
type ForwardMessage struct {
	...
	From_chat_id         int64 `json:"from_chat_id,omitempty"`
